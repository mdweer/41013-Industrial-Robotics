classdef LinearDM < RobotBaseClass
    ...
    properties(Access =public)   
        ...
        %> defaultRealQ 
        defaultRealQ  = [0,0,pi/4,0,pi/2,(-3*pi)/4, 0]; 
    end
...
%% CreateModel
        function CreateModel(self)   
            ...
            link(2) = Link('d',0.135429,    'a',0,      'alpha',0,  'offset',0, 'qlim',0);
            link(3) = Link('d',0.103+0.0362,    'a',0,      'alpha',-pi/2,  'offset',0, 'qlim',[deg2rad(-135),deg2rad(135)]);
            link(4)=  Link('d',0,        'a',0.135,  'alpha',0,      'offset',-pi/2, 'qlim',[deg2rad(5),deg2rad(80)]);
            link(5) = Link('d',0,        'a',0.147,  'alpha',0,      'offset',0, 'qlim',[deg2rad(-5),deg2rad(85)]);
            link(6) = Link('d',0,        'a',0.06,      'alpha',pi/2,  'offset',pi/2, 'qlim',[deg2rad(-180),deg2rad(180)]);
            link(7) = Link('d',-0.05,      'a',0,      'alpha',0,      'offset',pi, 'qlim',[deg2rad(-85),deg2rad(85)]);
...
%% TestMoveJoints
        ...
        function TestMoveJoints(self)
            self.TestMoveLinearDM();
        end

%% Test Move DobotMagician
function TestMoveLinearDM(self)
...
%% RealQToModelQ
        ...
        function modelQ = RealQToModelQ(realQ)
            modelQ = realQ;
            modelQ(3) = LinearDM.ComputeModelQ3GivenRealQ2and3( realQ(2), realQ(3) );
...
%% ModelQToRealQ
        ...
        function realQ = ModelQToRealQ( modelQ )
            realQ = modelQ;
            realQ(3) = LinearDM.ComputeRealQ3GivenModelQ2and3( modelQ(2), modelQ(3) );
        end
