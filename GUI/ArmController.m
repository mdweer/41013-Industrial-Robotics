    ...
    while length(qPath(:,1)) > stepCurrent
        if self.currentState == armState.EStop
            ...
            if self.gripperCloseFlag
                gripObjTr = self.currentTr * inv(self.gripTargetTr);
                nextVertices = (self.gripTargetVerts(:,1:3) * gripObjTr(1:3,1:3)') + gripObjTr(1:3,4)';
                set(self.gripTarget_h, 'Vertices', nextVertices);
                drawnow();
            end
            ...
            plot3(self.currentTr(1,4), self.currentTr(2,4), self.currentTr(3,4), '.-b');
            pause(delay);
    ...
    if self.currentState ~= armState.EStop
        ...
        for i = 1:length(err)
            if err(i) > self.ikErrorMax
                plot3(self.currentTr(1,4), self.currentTr(2,4), self.currentTr(3,4), 'x:r');
                goalReached = false;
            end
        end

        if goalReached
            plot3(self.currentTr(1,4), self.currentTr(2,4), self.currentTr(3,4), 'o:g');
        end
    end
end
